...
static s32 check_water_jump(struct MarioState *m) {
    ...
    if (m->input & INPUT_A_PRESSED) {
        ...
        if (
            (probe >= m->waterLevel - 80 && m->controller->stickY < -60.0f) ||
            (probe >= m->waterLevel - 150 && m->doubleA)
        ) {
            if (m->waterLevel - 80 > probe) m->pos[1] = m->waterLevel - 80;
...
s32 act_water_spin_attack(struct MarioState *m) {
...
s32 act_water_punch(struct MarioState *m) {
    if (m->dolphinPowers) return act_water_spin_attack(m);
