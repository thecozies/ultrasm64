...
s32 check_air_jump(struct MarioState *m) {
    if (m->pos[1] > m->waterLevel && m->canAirJump && m->input & INPUT_A_PRESSED) return TRUE;
    return FALSE;
}
...
s32 act_dive(struct MarioState *m) {
    ...
    switch (perform_air_step(m, 0)) {
        ...
        // CTODO: check it
            ...
            // Dive rollout
            if (++m->actionTimer > 6 && m->input & INPUT_A_PRESSED && m->pos[1] > m->waterLevel) {
...
s32 act_water_jump(struct MarioState *m) {
    // CTODO: Maybe this is too weird feeling?
    if (m->dolphinPowers) {
...
s32 act_ground_pound(struct MarioState *m) {
    ...
    if (m->actionState == 0) {
        ...
        // CTODO: Copy switch-case to dolphio if works
        switch (perform_air_step(m, 0)) {
            case AIR_STEP_NONE:
                if (check_air_jump(m)) return apply_air_jump(m);
                if (m->input & INPUT_B_PRESSED && m->pos[1] > m->waterLevel) {
...
s32 act_air_hit_wall(struct MarioState *m) {
    ...
    if (m->wall) m->marioObj->header.gfx.angle[1] = atan2s(m->wall->normal.z, m->wall->normal.x);
